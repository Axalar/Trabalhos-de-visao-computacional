...
% [cameraParams, ~, estimationErrors] = calCam(squareSize, 'video', {videoFileName}); % para video
[cameraParams, ~, estimationErrors] = calCam(squareSize, 'image', imageFileNames); % para imagens

%% Solid projection

% Load video
% vid = VideoReader(videoFileName); % para video

% for i=300:5:floor(vid.Duration*vid.FrameRate) % para video
for i=1:numel(imageFileNames) % para imagens

    % Load image
    im = iread(imageFileNames{i}, 'double'); % para imagens
    imgs = rgb2gray(im); % para imagens
    
%     % Load frame
%     im = read(vid,i); % para video
%     im = im2double(im); % para video
%     imgs = rgb2gray(im); % para video
